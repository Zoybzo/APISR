...
def super_resolve_img(
    ...
    img_lr = ToTensor()(img_lr).unsqueeze(0)  # Use tensor format
    if device != "cpu":
        img_lr = img_lr.cuda()
...
def super_resolve_video(
    ...
    encode_params = [
        ...
        "-preset",
        "medium",
    ]  # This is one of the best setting I used to use
    ...
    output_size = (
        int(width * scale / rescale_factor),
        int(height * scale / rescale_factor),
    )
    if has_audio:
        objVideoReader.audio.write_audiofile(
            temp_file_name + "/output_audio.mp3"
        )  # Hopefully, mp3 format is supported for all input video
        writer = FFMPEG_VideoWriter(
            output_path,
            output_size,
            original_fps,
            ffmpeg_params=encode_params,
            audiofile=temp_file_name + "/output_audio.mp3",
        )
    else:
        writer = FFMPEG_VideoWriter(
            output_path, size=output_size, fps=original_fps, ffmpeg_params=encode_params
        )
    ...
    for frame_idx, img_lr in enumerate(objVideoReader.iter_frames(fps=original_fps)):
        ...
        if rescale_factor != 1:
            img_lr = cv2.resize(
                img_lr,
                (int(width / rescale_factor), int(height / rescale_factor)),
                interpolation=cv2.INTER_LINEAR,
            )
        ...
        # Post process
        super_resolved_img = np.uint8(
            np.clip(
                torch.permute(super_resolved_img[0] * 255.0, (1, 2, 0))
                .cpu()
                .detach()
                .numpy(),
                0,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="GRL",
        help=" 'GRL' || 'RRDB' (for ESRNET & ESRGAN) || 'DAT' || 'CUNET' (for Real-ESRGAN) ",
    )
    parser.add_argument(
        "--weight_path",
        type=str,
        default="pretrained/4x_APISR_GRL_GAN_generator.pth",
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Downsample with same aspect ratio if the height/width (short side) is over the threshold limit, recommend to set as 720",
    )
    parser.add_argument(
        "--float16_inference",
        type=bool,
    ...
    if model == "GRL":
        generator = load_grl(
            ...
            scale=scale,
            device=device,
        )  # GRL for Real-World SR only support 4x upscaling

    elif model == "DAT":
        generator = load_dat(
            weight_path, scale=scale, device=device
        )  # GRL for Real-World SR only support 4x upscaling
